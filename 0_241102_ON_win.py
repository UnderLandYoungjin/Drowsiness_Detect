...
# 서버 URL 설정
upload_url = 'http://000.000.000.000:1200/upload'
        ...
        if recording_additional_15_seconds:
            if time.time() - additional_recording_start_time >= 15:
                ...
                if not os.path.exists(temp_folder):
                    os.makedirs(temp_folder)  # 폴더가 없으면 생성
                
                # 타임스탬프 설정 및 파일 이름 지정
                timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
                visual_filename = os.path.join(temp_folder, f"HOME_VISUAL_{timestamp}.mp4")
                original_filename = os.path.join(temp_folder, f"HOME_ORIGINAL_{timestamp}.mp4")
