...
def save_and_send_video(frames, frame_size):
    ...
    visual_filename = f"./CAL_01_V_{timestamp}.mp4"
    original_filename = f"./CAL_01_O_{timestamp}.mp4"
    ...
    if not out_visual.isOpened() or not out_original.isOpened():
        print("비디오 저장기 초기화 실패")
        return
    ...
    try:
        ...
        try:
            with open(visual_filename, 'rb') as f:
                res1 = requests.post(upload_url, files={'file': f})
                print("전송1 응답:", res1.status_code)
            with open(original_filename, 'rb') as f:
                res2 = requests.post(upload_url, files={'file': f})
                print("전송2 응답:", res2.status_code)
    ...
    with mp_face_mesh.FaceMesh(
        ...
        try:
            while cap.isOpened():
                ...
                # 프레임 수직 또는 수평 플립 (예: 수직 플립)
                frame = cv2.flip(frame, 1)  # 0: 수직, 1: 수평, -1: 수직 + 수평 모두
