...
font_small = ImageFont.truetype(font_large.path, 5) if hasattr(font_large, 'path') else ImageFont.load_default()
...
def draw_status_box(draw, frame_width, frame_height, drowsy_status, seatbelt_status, ear_value=0, distance=0, belt_conf=0):
    # 박스 크기와 위치 설정 (높이 증가)
    box_width = 180
    box_height = 170  # 높이 추가 증가
    margin = 10
    x = margin
    y = margin
    ...
    # 눈감김 비율 계산 (EAR을 퍼센트로 변환)
    eye_close_percent = max(0, min(100, (1 - ear_value / 0.3) * 100))  # EAR 0.3을 기준으로 비율 계산
    ...
    for offset in [(-1,-1), (-1,1), (1,-1), (1,1)]:
        ...
        draw.text((x_text + offset[0], y_text4 + offset[1]), f"눈감김: {eye_close_percent:.1f}%", 
                  font=font_large, fill=(0, 0, 0))
        draw.text((x_text + offset[0], y_text5 + offset[1]), f"전방주시: {100-min(100, distance/2):.1f}%", 
                  font=font_large, fill=(0, 0, 0))
    ...
    attention_color = (0, 255, 0) if distance <= 40 else (255, 0, 0)
    ...
    draw.text((x_text, y_text5), f"전방주시: {100-min(100, distance/2):.1f}%", 
        ...
        try:
            while cap.isOpened():
                ...
                draw_status_box(draw, frame_width, frame_height, drowsy_detected, seatbelt_detected, 
                              current_ear, current_distance, current_belt_conf)
